...
} = normalize(postsFromCategory7, list);
...
describe('Connection › List', () => {
  ...
  test('Get list shape when entity is not ready', () => {
    ...
    expect(connection.list('category', 7).entity.pagedLink(7)).toBe(
      '/?cat=7&paged=7',
    );
    expect(connection.list('latest', 'post').entity.pagedLink(7)).toBe(
      '/page/7/',
    );
  ...
  test('Get list entity shapes after adding a page', () => {
    ...
    expect(
      connection.list('category', 7).entities.map(entity => entity.id),
    ).toEqual(results);
    expect(
      connection
        .list('category', 7)
    ...
    expect(connection.list('category', 7).entities[0].title).toBe(
  ...
  test('Get list entity shapes after adding two pages', () => {
    ...
    expect(connection.list('category', 7).entities[5].title).toBe(
      'Machu Picchu',
    );
    expect(connection.list('category', 7).page(2).entities[0].id).toBe(30);
    expect(connection.list('category', 7).page(2).entities[0].title).toBe(
      'Machu Picchu',
    );
    expect(connection.list('category', 7).pages[1].entities[0].id).toBe(30);
    expect(connection.list('category', 7).pages[1].entities[0].title).toBe(
      'Machu Picchu',
    );
  });
