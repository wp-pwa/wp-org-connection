...
test('Convert single', () => {
  ...
  expect(entities.single[60].id).toBe(60);
  expect(entities.single[60].slug).toBe('the-beauties-of-gullfoss');
...
test('Convert list of singles', () => {
  const { entities } = normalize(category7, singles);
  expect(entities.single[57].id).toBe(57);
  expect(entities.single[57].slug).toBe('shinjuku-gyoen-national-garden');
