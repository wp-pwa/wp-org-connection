...
export const discoverUrl = connection => function* discoverUrlSaga({ firstFolder, lastFolder }) {
  try {
...
export function* redirectAfterDiscover({ postType, taxonomy, id }) {
  let key = '';
  if (postType) if (postType === 'post') key = '?p=';
    else if (postType === 'page') key = '?page_id=';
    else key = `?${postType}= `;
  else if (taxonomy) if (taxonomy === 'category') key = '?cat=';
